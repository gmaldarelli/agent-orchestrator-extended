...
function CreateProjectFlow({
	...
	const createProject = async (selection: CreateProjectAgentSelection) => {
		...
		try {
			...
			if (selectedKind === "single_repo" && (code === "NOT_A_GIT_REPO" || code === "PROJECT_UNBORN")) {
				const confirmed = window.confirm(
					`Set up Git for ${selectedPath}? AO will initialize Git if needed, stage this folder's files, and create the first commit before importing it.`,
				);
				if (!confirmed) {
					setError(message);
					return;
	...
	return (
		<>
			{children({
				choosePath: () => setModePickerOpen(true),
			...
			<CreateProjectModeDialog
				disabled={isChoosingPath || isCreating || isInitializing}
				open={modePickerOpen}
				onOpenChange={(open) => !isChoosingPath && !isCreating && !isInitializing && setModePickerOpen(open)}
			...
			<CreateProjectFolderDialog
				...
				onOpenChange={(open) => {
					if (!isChoosingPath && !isCreating && !isInitializing) {
